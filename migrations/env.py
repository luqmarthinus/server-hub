...
# Import your SQLAlchemy Base (this will be created in Phase 1)
...
def run_migrations_online() -> None:
    ...
    # Get the sync database URL from the environment (preferred) or fallback to alembic.ini
    database_url = os.getenv("SYNC_DATABASE_URL")
    if not database_url:
        database_url = config.get_main_option("sqlalchemy.url")
        if not database_url:
            raise ValueError(
                "No database URL found. Set SYNC_DATABASE_URL environment variable "
